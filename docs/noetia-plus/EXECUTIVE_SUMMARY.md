...
**ADR candidates (§45):** semantic-index technology (pgvector — highest priority); AI provider abstraction; KnowledgeAsset model; permission architecture; feature-entitlement architecture; usage metering; conversation retention; chunking strategy; hybrid-fusion/rerank. Listed for future ADRs; none finalized here.
...
**Caveat (not a blocker to *review*, but a blocker to *build*):** three inputs must be resolved before implementation begins — **(a) verified AI provider prices** (to confirm any retail price yields a Healthy COGS ratio), **(b) the copyright/quotation policy** (legal), and **(c) the grounded-vs-general-knowledge decision**. These are decisions, not unknowns in the architecture — the system is designed to absorb any of the plausible answers via configuration. Until they're set, do not start E0/E1.
