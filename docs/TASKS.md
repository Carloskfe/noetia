...
- [ ] **RESIDUAL constant sync offset — highlight still runs ~5–8 phrases AHEAD of the audio (added 2026-07-30, from Carlos)** — *distinct from the 2026-07-12 merge-drift fix above.* That fix removed the **cumulative** drift (per-chapter merge offsets); this is a **constant, non-growing** shift that remains: audio ~5 phrases behind in some books, ~8 in others, gap does not grow. Two confirmed contributing mechanisms:
  ...
  **Re-seed plan (applies the `5700bbe` aligner fix to live maps — Reader #1; runs on prod via SSH, Carlos):**
  ...
  - **✅ EXECUTED ON PROD 2026-07-31:** `--apply` re-seeded **58 books, 0 failed**, **18 correctly skipped** (matched the dry run). No free book dropped coverage (floor = Filipenses 90.7%). Served maps verified via `--stored` diagnose: Marianela / Génesis / Romeo y Julieta all `median 0s / OK`. The onset fix (`5700bbe`) is live across the free library. The 18 skips = the known problem set: 5 edition-mismatch (<60% cov: Cartas a Lucilio, El Príncipe, Meditaciones, Meditations, The Art of War), 6 large-offset/order-broken (Don Quijote I/II, El Sombrero, María, The Time Machine, Viaje al Centro), 7 accumulating-drift (Dracula, Isaías, La Divina Comedia, La Edad de Oro, La Odisea, Orgullo y Prejuicio, Dorian Gray). **Of the skips, only 4 are FREE — all "accumulating drift": `Dracula, Isaías, La Divina Comedia, La Edad de Oro`.** **TRIAGED 2026-08-01 via `diagnose-sync --profile`** (offset by book-position thirds separates duplicate/versed-text ARTIFACT from REAL drift):
    ...
    - *Follow-up left:* the two real-drift books' final third (above). Old spot-listen note superseded by the profile triage. The 7 non-free skips have no reader impact — leave for the text-source backlog.
