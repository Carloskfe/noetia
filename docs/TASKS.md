...
- [ ] **Free-library promotion candidates (2026-07-12 audit):** 9 books pass ≥90% but are `isFree=false` (culled pre-fix, never re-promoted) — As a Man Thinketh, How to Live on 24 Hours a Day, The Game of Life, The Science of Getting Rich, Up from Slavery (EN); La Edad de Oro, Lazarillo de Tormes, Pepita Jiménez, Platero y yo (ES). Set `isFree=true` to grow the beta library (curation call). **The Call of the Wild** is the one EN book with no VTT (`auto` source) — needs a transcription first.
