...
- [ ] **RESIDUAL constant sync offset — highlight still runs ~5–8 phrases AHEAD of the audio (added 2026-07-30, from Carlos)** — *distinct from the 2026-07-12 merge-drift fix above.* That fix removed the **cumulative** drift (per-chapter merge offsets); this is a **constant, non-growing** shift that remains: audio ~5 phrases behind in some books, ~8 in others, gap does not grow. Two confirmed contributing mechanisms:
  ...
  2. **Per-segment LibriVox credits (Carlos's field insight).** Each merged chapter re-introduces spoken credits/announcements ("This is a LibriVox recording…", section intro/outro) that are **in the audio but absent from the book text**. During those spans there is no text phrase to advance to, so the **highlight should HOLD static on the last real phrase while the audio keeps playing**; today the proportional model smears those extra words across the chapter, adding to the constant offset. This compounds per segment, which is why merged multi-chapter free-library books show the larger 8-phrase gap. **Fix path:** detect announcement spans (words with no text match between two aligned phrases) and (a) exclude them from the proportional word-count so they don't shift estimates, and (b) in the reader, keep the current phrase highlighted (not advance) while `currentTime` is inside an unmatched gap.
