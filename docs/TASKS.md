...
- [ ] **Audio progress bar reflects the chapter, not the whole book (added 2026-07-12, from Carlos)** — on multi-chapter/multi-section books the scrubber's total length maps to the *current chapter/section*, so the fill doesn't reflect real progress through the whole book (confusing vs where you actually are in the text). Expected: the bar spans the **entire book audio** end-to-end. Likely cause: those books are still served as **per-chapter audio** (or the player computes `duration`/`currentTime` per segment) instead of the single concatenated MP3 the self-host migration produces — so `duration` = one chapter. Check: (a) do the affected books have a single `audioStreamKey` MP3 vs per-chapter files? (b) reader scrubber uses `audio.duration`/`audio.currentTime` (web reader `<input type=range max={duration}>`, mobile equivalent) — confirm `duration` is the full-book length. Fix: guarantee whole-book concatenated audio (or aggregate segment durations into a book-level timeline) so the bar = real book advance. Reader #1.
...
- [x] **Library nav bar disappears** — FIXED (`26a8a51`). Root cause: the logged-in app had **no top bar on any tab** (Library/Discover/Profile just rendered a per-page title + the bottom tabs). Added a sticky `AppTopBar` (Noetia wordmark home link + profile + account/settings) to the `(library)` shell so every main tab has consistent top chrome. **Verify on prod after web deploy.**
  - **Follow-up:** the `(clubs)` route group has **neither** top nor bottom nav (tapping the Clubs tab currently drops the whole nav shell). Give clubs the same `AppTopBar` + `BottomNav` — but exclude the full-screen Escucha Juntos live-session page (needs a nested layout so the top/bottom chrome doesn't cover the live UI).
