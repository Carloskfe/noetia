...
- [ ] **SPEC — Final-third sync drift: La Divina Comedia & La Edad de Oro (added 2026-08-01)**
  ...
  - **Acceptance:** `diagnose-sync --realign --profile` shows **all three thirds median ≈ 0** (no ramp); `reseed-sync.sh --apply --only "<Title>"` then passes the gate; spot-listen the final third. The 7 non-free skips have no reader impact — leave for the text-source backlog.
