...
- [x] **In-reader progress status (web) — DONE** — reader top bar now shows a live progress indicator: a thin bar pinned to the header's bottom edge + a `%` readout (sm+). Tracks **scroll position while reading** and the **active phrase index while listening** (Escucha Activa/audio). `ReaderTopBar` gained a `progress` prop; the page computes it and reads the scroll fraction O(1)/rAF-throttled. i18n web en/es; jsdom test on ReaderTopBar. *(Follow-ups: persist reading position from scroll — today silent reading doesn't save position, only audio does; "page N of M" once pagination (item 1) lands; mobile reader indicator.)*
