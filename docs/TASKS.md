...
- [ ] **RESIDUAL constant sync offset — highlight still runs ~5–8 phrases AHEAD of the audio (added 2026-07-30, from Carlos)** — *distinct from the 2026-07-12 merge-drift fix above.* That fix removed the **cumulative** drift (per-chapter merge offsets); this is a **constant, non-growing** shift that remains: audio ~5 phrases behind in some books, ~8 in others, gap does not grow. Two confirmed contributing mechanisms:
  1. **Aligner early-window bias (confirmed, `phrase-aligner.ts`).** `scoreWindow` does an *unanchored* subsequence match in a window `ceil(n*1.5)+5` words wide and the search keeps the **earliest** offset that reaches a full match (`if (score === 1) break`, forward scan). Any offset a few words before the true onset still contains the whole phrase as a subsequence, so `best.pos` — and thus `startTime` — is set systematically **early**. The new diagnostic measures this component at a **constant ~1 phrase early (Marianela, Lazarillo), drift slope ≈ 0** — exactly a fixed shift, larger on books with longer phrases. **Fix path:** anchor the match on the phrase's first token (like `sync-diagnostics.ts` `anchoredScore`) so `startTime` lands on the real onset; re-seed affected books; re-diagnose (target median offset ≈ 0).
  ...
  - **Also surfaced:** in `--stored` mode Marianela shows a ~10 000 s scale gap between the **live DB sync map** and the **committed merged VTT** — i.e. the committed VTT is not the same-timescale transcript the DB map was built from (expected: `/app/transcriptions` is wiped/re-merged each deploy). Harmless to the reader today but means the committed VTTs are not a faithful record of the served maps. Worth reconciling when re-seeding.
