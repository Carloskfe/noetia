...
>
> **⏳ OPEN ACTION FOR CARLOS (added 2026-08-01):**
> - [ ] **Confirm La Divina Comedia by ear on prod** — the sync-offset fix is shipped + reseeded to 60 books; only La Divina Comedia is unresolved. Automated tooling localized a drift to **Paradiso Canto XIII (~80% in; first 80% verified perfect)** but can't tell a diagnostic false-positive from a real prose-text-vs-recording divergence (Paradiso's repeated verse defeats the locator). **Action:** open La Divina Comedia on noetia.app → Escucha Activa → Paradiso XIII, listen ~30s. Highlight tracks audio ⇒ mark resolved (optionally reseed for the onset fix); highlight is off ⇒ real per-book text/audio work (low priority — 1 free book). Full detail: sync backlog spec below + `docs/whisper-sync-troubleshooting.md §14`. *(Optional: same `diagnose-sync --profile` on La Edad de Oro for its onset canto.)*
>
...
> - [x] **Shared quote cards are invite pages (migration 065) — verified live on prod 2026-07-19** — "Copy link" now shares a public `/s/<id>` landing page (persisted `shares` record) instead of a bare PNG: shows the card + quote, a book-first CTA (`Lee «Book» en Noetia` → reader) + `Descubre Noetia`, and full OG/Twitter tags for rich link previews (bilingual via Accept-Language). Download/publish still use the PNG. Acquisition funnel: reader sends logged-out viewers to `/login?next=…`; login honours a safe internal `next`. **Real-share prod test (`/s/8ptyfA3Ugm`) caught a prod-only bug — SSR fetched unset `INTERNAL_API_URL` → fell back to `localhost:4000` (unreachable in-container) → every invite page 404'd; fixed by defaulting the server-side base to `api:4000` (`6852481`), re-verified 200 + correct OG/CTA.** ✅
>
...
> - [x] **BUG — Club creation book search returns nothing (fixed 2026-07-19, `690597a`)** — root cause was `GET /books` silently ignoring the `search` + `limit` query params the club picker sends, so it returned an unfiltered list (a typed title never narrowed results, and gated-out titles were absent). `BooksService.findAll` now applies a case-insensitive title/author `ILIKE` filter and `take(limit)`, behind the same ≥90% quality gate. Verified against dev Postgres. ✅
>   - [ ] *Follow-up (nice-to-have):* search is case-insensitive but **not accent-insensitive** (`galdos` won't match `Galdós`). For Spanish typeahead, add the Postgres `unaccent` extension + an unaccented index, or normalize the term both sides.
> - [x] **BUG — Share preview doesn't match downloaded image (text colour) (fixed 2026-07-19, `46f4693`)** — two causes: (1) auto-contrast diverged — the web preview averages per-colour luminances while image-gen averaged RGB channels then took luminance, so on gradients they picked opposite text colours; ShareModal now always forwards the exact resolved colour, making the preview authoritative. (2) photo backgrounds forced white and ignored a user override; `render_card` now honours an explicit override over images. Tests added (`test_base.py`). ✅
>   - [ ] *Not addressed here — quote text **size**:* preview font-size is a 260px-wide CSS approximation vs the 1080px render. `textScale` (S/M/L) is forwarded and clamped identically both sides, so the relative step is consistent, but the absolute preview-to-render size ratio hasn't been calibrated. Revisit if it reads as off.
> - [x] **Paged reading mode → default view (done 2026-07-19, `159391f`)** — product decision: paged is now the standard layout for everyone (`DEFAULTS.readingLayout = 'paged'`). A reader who switches to scroll has it remembered (localStorage `reader-layout`) and it wins over the default, so no re-enabling. Paged engages only for synced books; others fall back to scroll. Top-bar toggle remains the switch. ✅
> - [x] **Paged mode — select & save quotes from the page (done 2026-07-19, `f41a388`)** — the paged container now wires the same reading-mode-gated pointer selection handlers as the scroll column, so selecting a passage opens the fragment popover and saves a Fragmento. ✅
> - [x] **Paged mode — Kindle-like visual polish (done 2026-07-19, `2b3ec82`)** — centred ~620px measure (margins widen on large screens), generous vertical padding, justified + hyphenated text with first-line indents and no inter-paragraph gap, and a refined footer (slim progress track + subtle page indicator). *Visual — pending eyeball on prod; easy to tune (measure width / justify / indent / footer).* ✅
...
- [ ] **RESIDUAL constant sync offset — highlight still runs ~5–8 phrases AHEAD of the audio (added 2026-07-30, from Carlos)** — *distinct from the 2026-07-12 merge-drift fix above.* That fix removed the **cumulative** drift (per-chapter merge offsets); this is a **constant, non-growing** shift that remains: audio ~5 phrases behind in some books, ~8 in others, gap does not grow. Two confirmed contributing mechanisms:
  1. **Aligner early-window bias (confirmed → FIXED in code `5700bbe`, `phrase-aligner.ts`).** `scoreWindow` did an *unanchored* subsequence match in a window `ceil(n*1.5)+5` words wide and the search kept the **earliest** offset that reached a full match (`if (score === 1) break`, forward scan). Any offset a few words before the true onset still contains the whole phrase as a subsequence, so `best.pos` — and thus `startTime` — was set systematically **early**. The diagnostic measured this at a **constant ~1 phrase early (Marianela, Lazarillo), slope ≈ 0**. **Fix shipped:** new `matchSpan()` reads timing from the first/last phrase tokens that actually matched inside the chosen window, not the window edge; search + EMA drift unchanged. Post-fix `diagnose-sync --realign` = **0s / verdict OK** on both books; 14 aligner tests green (+1 regression). **⚠ Not yet live for readers — needs a re-seed (plan below).**
  2. **Per-segment LibriVox credits (Carlos's field insight) — INVESTIGATED 2026-07-31: already handled once cause 1 is fixed; no separate aligner change needed.** Each merged chapter re-introduces spoken credits ("This is a LibriVox recording…", section intro/outro) that are in the audio but absent from the text. Two concerns, both measured/resolved:
     - **Aligner (proportional-estimate skew):** *empirically absorbed.* With the cause-1 fix, `diagnose-sync --realign` returns **median 0 / verdict OK even on the worst-credit books** — Génesis (946 leading credit words / 393 s) and Crimen y Castigo (905 words / 431 s). The anchored `matchSpan` + existing EMA drift-correction + skip-and-continue already handle the per-boundary credit steps (LibriVox credits are ~20–40 words/chapter, well inside `MAX_DRIFT`). Building a two-pass credit-exclusion aligner would add regression risk for **zero measured benefit** — not pursued. (The non-OK many-chapter books are *edition mismatch* — Don Quijote 59% — or *structural drift* — La Divina Comedia 76%; separate text problems the reseed gate already refuses, not credits.)
     - **Reader (hold highlight static):** *already correct.* `reader-utils.ts phraseAt` returns the **last-started** timed phrase, so during a credit gap (`currentTime` past the last phrase's `endTime`, before the next phrase's `startTime`) the highlight **holds on the last real phrase** and the audio plays on — exactly the required behaviour. The highlight *jumping* during credits was itself a **symptom of cause 1** (early `startTime`s landing inside the gap), fixed by `5700bbe`. Locked with a regression test (`reader-utils.spec.ts` — "holds the last phrase static through a long inter-chapter announcement span"). Leading credits before phrase 0 (e.g. Génesis's 393 s) correctly show no highlight until narration begins.
  - **Diagnostic tool shipped (this change):** `services/api/src/ingestion/diagnose-sync.ts` + pure core `sync-diagnostics.ts` (7 unit tests). Run per book: `docker compose exec api npx ts-node -r tsconfig-paths/register src/ingestion/diagnose-sync.ts --book "Title" --transcript /app/transcriptions/<slug>.merged.vtt --realign`. Reports edition coverage (are text & audio the same edition?), leading/embedded extra words (the credits), median offset in **seconds and phrases**, and drift slope (constant vs growing) with a verdict. **Use `--realign`** (splits the stored text and aligns to the committed VTT — apples-to-apples); `--stored` compares the live DB map against the committed VTT and is only valid when they share a timescale.
  - **Also surfaced:** in `--stored` mode Marianela shows a ~10 000 s scale gap between the **live DB sync map** and the **committed merged VTT** — i.e. the committed VTT is not the same-timescale transcript the DB map was built from (expected: `/app/transcriptions` is wiped/re-merged each deploy). Harmless to the reader today but means the committed VTTs are not a faithful record of the served maps. Must be reconciled as part of the re-seed (below).

  **Re-seed plan (applies the `5700bbe` aligner fix to live maps — Reader #1; runs on prod via SSH, Carlos):**
  - **P0 — Pre-flight.** Take a Contabo snapshot (sync maps live in Postgres `sync_maps`; nightly 2 AM backup is the second net). For every target book, verify the committed merged VTT matches the **served MinIO audio timeline** before trusting it: `last-cue-end(VTT) ÷ ffprobe-duration(MP3)` within the §12 guardrail band. Any mismatch ⇒ re-merge that book first via the existing `drift-realign` path (real audio-duration offsets) — do **not** re-seed from a stale VTT (that is the source of the ~10 000 s `--stored` gap).
  - **P1 — Dev acceptance gate (per book).** Run `diagnose-sync --realign`. Require: edition coverage ≥ 0.90 **and** median offset ≈ 0 **and** slope ≈ 0. Coverage < 0.60 ⇒ edition mismatch — **skip the re-seed**, route to the text-source backlog instead (re-seeding won't help). This is the go/no-go per title.
  - **P2 — Re-seed free library first (66 whisper books).** `seed-sync-whisper --book "T" --transcript <vtt>` (idempotent — overwrites the map). After each: recompute `syncCoverage` (`sync-quality-report`) and hold the **≥ 0.90 gate** — if a re-seed drops a book below gate, restore that one map from snapshot and investigate before continuing. Then re-run `seed-search`.
  - **P3 — Verify on prod.** Spot-listen 3–4 books in Escucha Activa (a short novel, a Bible book, a long merged book) — highlight must track audio. Re-run `diagnose-sync --stored` on prod for the re-seeded titles: median offset should now be ≈ 0 (live map == fixed realign).
  - **P4 — Non-free long tail.** Re-seed the remaining whisper books for consistency (no reader impact; lower priority).
  - **Rollback:** re-seed is per-book + idempotent, so a bad title can simply be re-seeded or restored individually from the snapshot / nightly backup.
  - **✅ EXECUTED ON PROD 2026-07-31:** `--apply` re-seeded **58 books, 0 failed**, **18 correctly skipped** (matched the dry run). No free book dropped coverage (floor = Filipenses 90.7%). Served maps verified via `--stored` diagnose: Marianela / Génesis / Romeo y Julieta all `median 0s / OK`. The onset fix (`5700bbe`) is live across the free library. The 18 skips = the known problem set: 5 edition-mismatch (<60% cov: Cartas a Lucilio, El Príncipe, Meditaciones, Meditations, The Art of War), 6 large-offset/order-broken (Don Quijote I/II, El Sombrero, María, The Time Machine, Viaje al Centro), 7 accumulating-drift (Dracula, Isaías, La Divina Comedia, La Edad de Oro, La Odisea, Orgullo y Prejuicio, Dorian Gray). **Of the skips, only 4 are FREE — all "accumulating drift": `Dracula, Isaías, La Divina Comedia, La Edad de Oro`.** **TRIAGED 2026-08-01 via `diagnose-sync --profile`** (offset by book-position thirds separates duplicate/versed-text ARTIFACT from REAL drift):
    - **Dracula** (thirds 0·0·0, 1/134 outliers) + **Isaías** (0·0·0, 7/138) = **ARTIFACT** — bulk *and* ending aligned; the drift verdict is versed/duplicate-line noise. **Reseeded** (onset fix applied): Isaías 1377/1388 aligned; Dracula via direct seed. *(Note: `--stored` still prints "accumulating drift" for these — the top-line verdict is fooled by the same duplicate-line outliers; trust the thirds, not the headline.)*
    - **La Divina Comedia** (0·0·**+1518s**, cov 82.2%) + **La Edad de Oro** (0·0·**+1789s**, cov 92.4%) = **REAL drift, localized to the FINAL THIRD** (first two-thirds perfect; audio walks ~25–30 min off the text by the end). A reseed won't fix it — the merged VTT drifts there. → **text-source backlog: re-examine the closing chapters' VTT merge / re-transcribe the final sections / check the late-section text edition.** Not a sync-tooling issue.
    - *Follow-up left:* the two real-drift books' final third — see the dedicated spec below. Old spot-listen note superseded by the profile triage.

- [ ] **SPEC — Final-third sync drift: La Divina Comedia & La Edad de Oro (added 2026-08-01)**
  - **Priority:** low. Both are FREE and pass the coverage gate (82.2% / 92.4%), so they stay live; only the **closing ~third** drifts — first two-thirds play in sync. Not a blocker.
  - **Symptom:** `diagnose-sync --realign --profile` → median offset **0s in the first and middle thirds, but +1518s (Divina) / +1789s (Edad de Oro) in the last third**; audio walks ~25–30 min off the text by the end. The onset fix + reseed can't help — the merged VTT's own timeline drifts in the closing sections.
  - **Likely root causes** (see whisper-sync-troubleshooting §10/§12): (1) grouped/mismatched transcription — the closing audio files didn't pair 1:1 with VTT segments at merge time, so late offsets fell back to the +2s gap heuristic instead of real ffprobe durations (same failure mode as La Odisea); (2) a missing/extra chapter in the merge shifting everything after it; (3) a localized edition mismatch — the text's closing sections (Paradiso / late Martí pieces) differ from the recording.
  - **Diagnose first (localize the break):**
    1. Full profile (no grep) to read the outlier phrase indices — the first index where Δ jumps marks where drift starts: `dc exec -T api node dist/ingestion/diagnose-sync.js --book "La Divina Comedia" --transcript /app/transcriptions/la-divina-comedia.merged.vtt --realign --profile`
    2. Map that phrase index → the nearest heading (canto/chapter) to name the first bad section.
    3. Compare counts: # concatenated audio files (archive.org item / `audioStreamKey`) vs # VTT segments merged (merge manifest) vs # text chapters — a tail mismatch confirms cause 1/2.
    4. Confirm whether real ffprobe durations were used for the closing files at merge time (drift-realign log).
  - **Fix (pick per finding):** merge-offset drift → re-run the VTT merge with correct per-file durations for the closing files, re-seed (no re-transcription). Missing/extra chapter → fix the merge manifest ordering, re-merge, re-seed. Localized edition mismatch → re-transcribe the closing audio via Colab (`whisper-book.py`), merge, re-seed; or correct the closing text to match the recording.
  - **Acceptance:** `diagnose-sync --realign --profile` shows **all three thirds median ≈ 0** (no ramp); `reseed-sync.sh --apply --only "<Title>"` then passes the gate; spot-listen the final third.
  - **La Divina Comedia — LOCALIZED 2026-08-01 (prod):** decile profile `0 0 0 0 0 0 0 0 1846 39848`; drift onset **~phrase 4110, heading "CANTO DECIMOTERCIO" = Paradiso Canto XIII** (canto 80/100, ~80% in). First 80% perfectly aligned. **Not a merge/missing-audio issue:** the merged VTT reaches **12:25:43** (full ~12.5h audiobook, 8797 cues) — audio for the closing cantos is present and timed, so a re-merge/re-transcribe won't help, and `--realign` reproduces the drift so a plain reseed won't either. Text around the onset (dumped) is **clean prose** — no non-narrated block, no duplication, no malformed heading — but it is **Paradiso rendered as prose, dense with repeated liturgical/numeric lines** ("uno y dos y tres", "Amén", invocations). That is exactly the input that makes the `--profile` anchor locator least reliable (repeated lines mis-matched across cantos at the wide-band far end). **Two candidate realities, only distinguishable by ear:** (a) diagnostic false-positive — the served map actually plays fine (then: mark resolved, optionally reseed for the onset fix); (b) a real prose-text-vs-recorded-translation divergence in Paradiso (also fits the 82% coverage) — a genuine per-book text/audio project. **Decision: needs a human listen to Paradiso XIII on prod; automated tooling has reached its reliable limit here. Low priority (1 free book, first 80% perfect) — defer or de-list unless the listen shows a clear reader-facing break.** Same method + caveat likely applies to La Edad de Oro (run `--profile` to get its onset canto). The 7 non-free skips have no reader impact — leave for the text-source backlog.
  - **Tooling BUILT (`scripts/reseed-sync.sh`):** gated batch re-seed. Reuses `scripts/drift-realign.tsv` as the title→slug manifest (single source of truth), orders **free-library books first** (live `isFree` query), and runs P1's `diagnose-sync --realign` as a **pre-write gate** — only books passing coverage ≥ `--min-coverage` (60%) and |median offset| ≤ `--max-offset` (1 phrase) and not "accumulating drift" get written. **Dry run by default; `--apply` to write.** Prints a re-seeded/skipped/failed summary. Validated on dev: gate → apply → post-seed live map reads 0s in `--stored` (also resolves the committed-VTT/live-map timescale gap noted above). Run on prod from `/opt/noetia` after `git pull`; dev testing via the `COMPOSE`/`DIAGNOSE_CMD`/`SEED_CMD` env overrides documented in the script header.
  - **Out of scope here (separate item):** cause (2), the per-segment LibriVox credits — reader holds highlight static during unmatched announcement spans + exclude those words from the proportional estimate. The re-seed fixes cause (1) only; a residual phrase-or-two around chapter boundaries remains until cause (2) ships.
...
- [x] **v1 paged reading mode (web) — SHIPPED behind a default-off toggle** — `readingLayout` preference (`scroll` | `paged`, default scroll) + top-bar toggle. Plain reading of a synced book renders a paged view: CSS multi-columns one page wide, translated horizontally a page at a time; prev/next buttons, edge tap zones, arrow keys, page indicator; reflow-safe (recomputes pages + re-anchors current phrase on resize/font change). `phraseIndex` stays canonical — resumes at saved position, persists the current page's first phrase. Escucha Activa/audio stay on scroll (sync engine untouched). Shared `PhraseRenderer` extracted; paging math pure+unit-tested; columns+translate verified in headless Chromium.
  - [x] **Paged + audio sync — DONE** — paged layout now also covers Escucha Activa: as audio advances, the page auto-flips to keep the narrated phrase on screen (highlight already tracks `activePhraseIndex`). PagedReader fills its parent so it sits beside the audio sidebar (desktop) / above the control sheet (mobile); tap-to-seek + fragment capture preserved by confining page-turn zones to the page margin. Fixed a real page-mapping bug (inline phrases start mid-line → `round` over-counted; now `pageForOffset` = floor of absolute offset), caught via headless-Chromium verification.
...
- [x] **In-reader progress status (web) — DONE** — reader top bar now shows a live progress indicator: a thin bar pinned to the header's bottom edge + a `%` readout (sm+). Tracks **scroll position while reading** and the **active phrase index while listening** (Escucha Activa/audio). `ReaderTopBar` gained a `progress` prop; the page computes it and reads the scroll fraction O(1)/rAF-throttled. i18n web en/es; jsdom test on ReaderTopBar.
  - [x] **Persist reading position from scroll — DONE** — silent reading now records a resume point: a debounced, server-only scroll handler posts the topmost on-screen phrase via `POST /books/:id/progress`. The which-phrase choice is a pure tested helper (`resumePhraseIndex`) that skips headings/markers so the saved index is always narratable; it never touches `savedPhraseIndex`, so saving can't yank the scroll.
  - *(Remaining follow-ups: "page N of M" once pagination (item 1) lands; mobile reader indicator + scroll persistence.)*
