...
#### Scenario: Light background produces dark text
- **WHEN** `render_card` is called with `bg_colors=["#FFFFFF"]`
- **THEN** the rendered card uses dark navy text colour

#### Scenario: Dark background produces white text
- **WHEN** `render_card` is called with `bg_colors=["#000000"]`
...
The image-gen service SHALL expose `POST /generate` accepting JSON `{ text, author, title, platform, format?, font?, bgType?, bgColors? }`. It SHALL render the card using the specified parameters (with defaults: `format=post`, `font=lato`, `bgType=solid`, `bgColors=["#0D1B2A"]`), upload the PNG to MinIO `images/` bucket with a UUID filename, and return `{ url: <pre-signed URL> }` with HTTP 200.
...
- **WHEN** a POST request is sent with `{ text, author, title, platform: "instagram", format: "story", font: "playfair", bgType: "gradient", bgColors: ["#0D1B2A", "#1A4A4A"] }`
